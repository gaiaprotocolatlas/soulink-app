...
class Mint extends skydapp_common_1.View {
    ...
    async loadPrice() {
            ...
            for (const [address, type] of Object.entries(Config_1.default.discountNFTs)) {
                if (type === 1) {
                    promises.push((async () => {
                        ...
                        if (balance.gt(0)) {
                            discountPercent = await DiscountDBContract_1.default.getDiscountRate(user, utils_1.defaultAbiCoder.encode(["address"], [address]));
                            discountNFT = address;
                        }
                    })());
                }
                    ...
                    promises.push((async () => {
                        ...
                        if (balance.gt(0)) {
                            discountPercent = await DiscountDBContract_1.default.getDiscountRate(user, utils_1.defaultAbiCoder.encode(["address", "uint256"], [address, 0]));
                            discountNFT = address;
                        }
                    })());
                }
        ...
        this.priceDisplay.empty().appendText(ethers_1.utils.formatEther(price.mul(10000 - discountPercent).div(10000)));
