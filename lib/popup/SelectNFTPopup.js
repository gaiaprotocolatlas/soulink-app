...
export default class SelectNFTPopup extends Popup {
    ...
    constructor(currentContract, currentTokenId, select) {
        ...
        this.append(this.content = el(".select-nft-popup", el("h1", "Select NFT"), this.main = el("main", this.nftContainer = el(".nft-container"), this.loadMoreButton = el("a.load-more", "Load More", {
            click: async () => {
                const loading = el(".loading").appendTo(this.main, 1);
                const nfts = await NFTLoader.loadMore(AdminLayout.current.address);
                for (const nft of nfts) {
                    this.nftDisplays[`${nft.asset_contract.address}-${nft.token_id}`] = el("a.nft", new NFTDisplay(nft.image_thumbnail_url), el(".name", nft.name === null ? "" : nft.name), { click: () => this.selectNFT(nft.asset_contract.address, nft.token_id) }).appendTo(this.nftContainer);
                }
                if (nfts.length < 50) {
        ...
        })), el(".info-form", this.contractAddressInput = el("input", {
        ...
        })), el(".button-container", el("a.select", "Close", {
            click: () => this.delete(),
        }), el("a.select", "Select", {
            click: () => {
    ...
    async loadNFTs() {
        ...
        this.nftDisplays["empty"] = el("a.nft", el(".empty"), el(".name", "Empty"), { click: () => this.selectNFT(undefined, undefined) }).appendTo(this.nftContainer);
        for (const nft of nfts) {
            this.nftDisplays[`${nft.asset_contract.address}-${nft.token_id}`] = el("a.nft", new NFTDisplay(nft.image_thumbnail_url), el(".name", nft.name === null ? "" : nft.name), { click: () => this.selectNFT(nft.asset_contract.address, nft.token_id) }).appendTo(this.nftContainer);
        }
        if (nfts.length === 0) {
