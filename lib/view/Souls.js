        ...
        await Layout_1.default.current.ready(addressOrEns, async () => {
            if (this.closed !== true) {
                ...
                (async () => {
                    ...
                    const result = await fetch(`${Config_1.default.apiURI}/linked/${address}`);
                    const linkedAddresses = await result.json();
                    for (const address of linkedAddresses) {
                        new SoulDisplay_1.default(address, Layout_1.default.current.bio.color).appendTo(this.soulList);
                    }
                    if (this.closed !== true) {
                        loading.delete();
                    }
