            ...
            click: async () => {
                ...
                if (nfts.length === 0) {
                    ...
                    for (const nft of nfts) {
                        this.nftDisplays[`${nft.asset_contract.address}-${nft.token_id}`] = (0, skydapp_browser_1.el)("a.nft", nft.image_thumbnail_url.indexOf(".mp4") !== -1 ? (0, skydapp_browser_1.el)("video", { src: nft.image_thumbnail_url, defaultMuted: true, muted: true, autostart: true }) : (0, skydapp_browser_1.el)("img", { src: nft.image_thumbnail_url }), (0, skydapp_browser_1.el)(".name", nft.name === null ? "" : nft.name), { click: () => this.onNFT(nft.asset_contract.address, nft.token_id) }).appendTo(this.nftContainer);
                    }
                }
