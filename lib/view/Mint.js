...
export default class Mint extends View {
    ...
    constructor(params) {
        super();
        BodyNode.append(this.container = el(".mint-view", el("header"), el("main", el("header", "S O U L I N K"), el("section", new ResponsiveImage("img", "/images/mint-image.png"), el("section", el("h1", el("b", "Let’s"), " Link!"), el(".stepper", el(".step", "Pre-launch Discount Period"), el(".step.on", "Public Sale"), el(".step", "LFG⚡️")), el(".info", el("p.quantity", "Quantity : MAX 1 per wallet : ", el("span", el("b", "1"))), el("p.price", "Price: ", el("span", this.priceDisplay = el("b", "..."), "eth"))), this.help = el("p.help"), el("a.mint", "Mint", {
            click: async () => {
        ...
        }), el("p", "Soulink is a soulbound token; this means that once you purchase it, you cannot transfer or sell it to another wallet address.")))), el("footer", el("a", new ResponsiveImage("img", "/images/logo.png"), {
