...
export default class SoulsSetting extends View {
    ...
    async load() {
        await AdminLayout.current.ready(async () => {
            if (this.closed !== true) {
                let loading;
                AdminLayout.current.content.append(this.container = el(".souls-setting-view", this.toLinkContainer = el(".soul-list.to-link-container"), this.toAcceptContainer = el(".soul-list.to-accept-container"), this.linkedContainer = el(".soul-list.linked-container"), loading = el(".loading")));
                (async () => {
    ...
    async loadRequest(linked) {
        ...
        for (const request of requests) {
            if (request.accept === undefined &&
                ...
                const requestDisplay = new SoulDisplay(request.bio ?? { id: request.requester, links: [] }, AdminLayout.current.bio.color, el("a", el("i.fa-light.fa-check"), {
                    click: async () => {
                        if (AdminLayout.current.address !== await Wallet.loadAddress()) {
                            new Alert(`Whoa! Looks like you’ve signed in with another account. Please sign back in with ${AdminLayout.current.address} to continue.`);
                        }
